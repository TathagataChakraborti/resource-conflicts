...
#                        print "Yo Bro, lay off my meds for time " + str(t) + ' !!!'
    ...
    for r in res:
        for t in range(0,T):
            for v in solnList:
                if v.VarName.split('_')[0] == r and v.VarName.split('_')[-1] == str(t) and r in v.VarName:
                    cc = v.X*globalVAR.resource_profiles_use[v.VarName.split('_')[0],'IN_USE'][t]
                    conflict += cc
                    count += v.X
    ...
    for r in res:
        for t in range(0,T):
            for v in solnList:
                if 'res_' in v.VarName and v.VarName.split('_')[-1] == str(t) and r in v.VarName:
                    ffs = v.VarName.split('_')[1]+'_'+v.VarName.split('_')[2]+'_'+v.VarName.split('_')[3]
                    pp = v.X*globalVAR.resource_profiles_grnd[ffs][t]
                    prob += pp
                    if pp > 0:
			count += 1
...
def run_ip(domainFile, problemFile, T, num_kits=2):
